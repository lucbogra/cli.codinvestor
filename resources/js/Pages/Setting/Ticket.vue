...
const submitTicket = () => {
  ticket.post(route('user.store.ticket'), {
    ...
    onSuccess: () => {
      isLoading = false
      toastShow("Your ticket has been sent successfully. You will receive an answer soon", 'success')
      ticket.reset()
    },
    onError: () => {
      isLoading = false
      toastShow("Please check if all fields are filled", 'error')
    }
  ...
  <Layout title="Profile">
    <div class="max-w-6xl bg-white mx-auto sm:px-6 lg:px-8 p-8">
      ...
      <form @submit.prevent="submitTicket">
        ...
        <div class="mt-4">
          <button class="w-full btn-primary text-white p-2 rounded shadow-md">Send a Ticket</button>
        </div>
      </form>
    </div>
  </Layout>
